...
def update_chat(n_clicks, clear_clicks, n_intervals, user_message, chat_history, chat_history2, model_name):
    global streaming_done
    ...
    if triggered_id == "stream-interval":
        ...
        for msg in chat_history2:
            if "```" in msg["content"]:  # Check if it's a code block
                ...
                chat_history_display.append(dmc.Alert(msg["content"], title=msg["role"].capitalize(), color="violet" if msg["role"] == "user" else "blue"))
